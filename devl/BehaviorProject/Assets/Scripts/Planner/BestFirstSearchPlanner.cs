...
namespace POP
{
	 ...
	 interface IPlannerInterface<StateMap>
	{
		...
		public class DefaultAction
		{
			...
			public int hashID;//MAX ADDED
		};

		public class DefaultState
		{
			public ulong[] globalState;
	...
	class BestFirstSearchPlanner : IPlannerInterface<Dictionary<DefaultState, BestFirstSearchNode>> {
		    ...
		    public uint numNodesExpanded = 0;
...
/*
				if (x.action is BSTL.Planner.Action)
					Debug.Log ("generating transitions " + ((BSTL.Planner.Action) x.action).task);
				else
...
//				Debug.Log ("Count: " + possibleActions.Count);
	...
	//				Debug.Log ("Action: " + ((BSTL.Planner.Action) action).task + " Cost: " + action.cost);
...
//					Debug.Log ("");
	...
	//				Debug.Log ("next node: " + ((Action) nextNode.action).task);
	//				Debug.Log("Total Cost: " + newf);

	//				Debug.Log ("PLANNING STATE: " + Convert.ToString((long)action.state.globalState[0],2));
	//				Debug.Log ("PLANNING STATE: " + Convert.ToString((long)action.state.globalState[1],2));
	//				Debug.Log ("PLANNING STATE: " + Convert.ToString((long)action.state.globalState[2],2));
	//				Debug.Log ("PLANNING STATE: " + Convert.ToString((long)action.state.globalState[3],2));
			...
			do {
	            ...
	            foreach(KeyValuePair<DefaultState, BestFirstSearchNode> node in stateMap)
	            {
	              if(node.Key.Equals(s)){
					  a = node.Value.action;
	                  plan.Push(a);
						/*SmartInteraction A = a as SmartInteraction;//DELeTE
						if(A!=null){
					Debug.Log (A.ToString());//DELETE
						}*/
